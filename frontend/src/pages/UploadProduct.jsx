          ...
          <ul className="sidebar-steps">
            {visualSteps.map((label, index) => {
              ...
              return (
                <li
                  key={label}
                  className={['sidebar-step', completed ? 'sidebar-step--completed' : '', current ? 'sidebar-step--current' : ''].filter(Boolean).join(' ')}
                >
        ...
        <section className="onboarding-content">
          <div className="progress-header">
            <div className="progress-header-text">
              <span className="progress-step-count">Step {step} of 4</span>
              <span className="progress-step-caption">Guided listing creation, Amazon-style</span>
            </div>
          </div>
          ...
          <div className="upload-container">
            ...
            {step === 1 && <PhotoUpload onPhotosUploaded={handlePhotosUploaded} />}
            {step === 2 && (
              <VoiceRecorder onVoiceRecorded={handleVoiceRecorded} onBack={handleBack} />
            )}
            {step === 3 && listing && (
              <AIChat productContext={listing} onComplete={handleChatComplete} />
            )}
            {step === 4 && listing && (
              <ListingPreview
                listing={listing}
