...
function ProductCard({ product, onClick }) {
  ...
  return (
    <article
      className="group bg-card border border-border-gray rounded shadow-card transition-shadow duration-200 cursor-pointer flex flex-col hover:shadow-card-hover focus-ring focus:outline-none p-3"
      onClick={onClick}
    ...
    >
      <div className="relative w-full pt-[100%] overflow-hidden bg-card mb-2">
        ...
        />
        <div className="absolute top-1 left-1 flex items-center gap-1 bg-new-badge-bg text-new-badge-text py-0.5 px-1 rounded text-[11px] font-normal">
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {tags.map((tag, index) => (
              <span
                key={tag + index}
                className="bg-bg-light text-text-secondary py-0.5 px-1.5 rounded text-[11px]"
              >
                {tag}
              </span>
            ))}
